document.addEventListener('DOMContentLoaded', function() {
    ...
    /* ------- tsParticles live background initialization ------- */
    if (window.tsParticles) {
        tsParticles.load('tsparticles', {
            ...
            particles: {
                ...
                color: { value: ['#1f2937', '#2563eb', '#60a5fa'] },
                shape: { type: 'circle' },
                opacity: { value: 0.6 },
                size: { value: { min: 2, max: 6 } },
                move: { enable: true, speed: 1.2, direction: 'none', outMode: 'out' }
            },
            detectRetina: true
        });
    }
...
document.querySelectorAll('a[href^="#"]').forEach(function(anchor) {
    ...
    // Internship certificate buttons removed (no internships listed)
    ...
    /* ------- GSAP + ScrollTrigger animations for polished, MNC-like motion ------- */
    try {
        if (window.gsap && window.ScrollTrigger) {
            gsap.registerPlugin(ScrollTrigger);

            // Hero headline subtle parallax
            gsap.to('.hero__name', {
                y: -30,
                ease: 'none',
                scrollTrigger: {
            ...
            // Section subtle reveal using clip-path
        ...
        // If GSAP not loaded, silently ignore
        console.warn('GSAP/ScrollTrigger not initialized:', e);
    }
});